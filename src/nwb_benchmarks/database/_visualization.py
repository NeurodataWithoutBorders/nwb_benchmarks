...
class BenchmarkVisualizer:
    ...
    def add_mean_sem_annotations(self, value: str, group: str, order: List[str], **kwargs):
        ...
        for i, label in enumerate(order):
            if label in stats_df.index:
                ...
                plt.text(x=stats["max"], y=i, s=mean_sem_text, 
    ...
    def plot_benchmark_dist(self, df: pd.DataFrame, group: str, metric_order: List[str],
        filename: str, row: Optional[str] = None, sharex: bool = True, add_annotations: bool = True,
        kind: str = 'box', palette: str = 'Paired', catplot_kwargs: Optional[Dict[str, Any]] = None
    ):
    ...
    def plot_benchmark_slices_vs_time(self, df: pl.LazyFrame, group: str, metric_order: List[str],
        filename: Path, row: Optional[str] = None, sharex: bool = True):
    ...
    def plot_benchmark_heatmap(self, df: pl.LazyFrame, group: str, metric_order: List[str], ax: Optional[plt.Axes] = None) -> plt.Axes:
        ...
        for j, col in enumerate(heatmap_df.columns):
            ...
            ax.text(j + 0.5, i + 0.5, '     *', fontsize=20, ha='center', va='center',
                    color='black', weight='bold')
        
        return ax
            
    def _create_plot_kwargs(self, df, group: str, order: List[str], 
                        filename: Path, kind: str = 'box', **extra_kwargs) -> Dict[str, Any]:
    ...
    def plot_method_rankings(self, db: BenchmarkDatabase):
        ...
        axes[0] = self.plot_benchmark_heatmap(df=read_df, group="benchmark_name_operation", 
                                              metric_order=self.file_open_order, ax=axes[0])
        
        axes[1] = self.plot_benchmark_heatmap(df=read_df, group="benchmark_name_operation",
                                              metric_order=self.pynwb_reading_order, ax=axes[1])
        
        axes[2] = self.plot_benchmark_heatmap(df=slice_df, group="benchmark_name_test", 
                                              metric_order=self.slicing_order, ax=axes[2])
    ...
    def generate_all_plots(self, db: BenchmarkDatabase):
        ...
        self.plot_read_benchmarks(db, self.download_order, benchmark_type="time_download_lindi", col_name="benchmark_name_test", 
        ...
        self.plot_read_benchmarks(db, self.pynwb_reading_order, benchmark_type=benchmark_type, 
                                  network_tracking=True, suffix='pynwb')
